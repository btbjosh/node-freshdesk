...
Freshdesk = (function() {
  ...
  Freshdesk.prototype.getTicketsByEmail = function(email, callback){
      return this.get('/helpdesk/tickets/filter/requester/'+email+'?format=json', function(err, res, body) {
              var tickets = JSON.parse(body);
              if (tickets.length !== 0) {
                  return callback(tickets[0]);
              }
